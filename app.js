(() => {
  ...
  function renderLandMetrics() {
    const metrics = dashboard.land_metrics || {};
    const cards = [
      ['إجمالي مساحة الأرض', metrics.total_land_area, 'فدان'],
      ['المساحة المستهدفة لزراعة المجدول', metrics.target_medjool_area, 'فدان'],
      ['المساحة المستهدفة للزراعات الأخرى', metrics.target_other_crops_area ?? metrics.remaining_other_crops_area, 'فدان'],
      ['عدد نخيل المجدول المستهدف', metrics.target_medjool_palms, 'نخلة'],
      ['عدد نخيل المجدول المزروع', metrics.planted_medjool_palms, 'نخلة']
    ];
    el('landMetrics').innerHTML = cards.map(([label, value, unit]) => `
      <article class="land-metric-card">
        <span class="metric-accent"></span>
        <span class="metric-label">${label}</span>
        <span class="metric-value-row">
          <strong class="metric-value">${plainNumber(value)}</strong>
  ...
  function renderKpis(summary, full, isFiltered) {
    const cards = [
      ...
      ['العائدات', full.revenues || 0, 'ج.م', '']
    ];
    el('kpis').innerHTML = cards.map(([label, value, unit, cls]) =>
      `<article class="card kpi ${cls}"><span class="label">${label}</span><strong class="value">${unit === '%' ? pct(value) : money(value)}</strong><span class="unit">${unit === '%' ? 'من الميزانية المعروضة' : 'جنيه مصري'}</span></article>`
    ).join('');
  }
  ...
  function renderCumulative(tasks) {
    ...
    // The planned cumulative line should start from the already-spent pre-April actual
    // amount, then add the planned values from the following periods. This aligns
    // the planned curve with the project position at the beginning of April.
    const plannedBaseline = actual.length ? actual[0] : 0;
    const planned = series.map(month => plannedBaseline + (runningPlanned += month.planned));
  ...
  async function initLandMap() {
    const status = el('mapStatus');
    if (!el('landMap')) return;
    if (!window.L) {
      status.textContent = 'تعذر تحميل الخريطة التفاعلية. تحقق من اتصال الإنترنت.';
      status.classList.add('error');
      return;
    }

    const entrance = [28.248303, 28.897119];
    const map = L.map('landMap', {
      zoomControl: true,
    ...
    L.control.layers({ 'صور جوية': satelliteLayer, 'خريطة الشوارع': streetLayer }, null, { position: 'topright' }).addTo(map);
    L.control.scale({ imperial: false, position: 'bottomleft' }).addTo(map);

    const entranceMarker = L.circleMarker(entrance, {
      radius: 8,
      color: '#fffdf8',
      weight: 3,
      fillColor: COLORS.gold,
      fillOpacity: 1
    }).addTo(map).bindPopup('<div class="land-popup-title">نقطة الوصول إلى الأرض</div><div dir="ltr">28.248303, 28.897119</div>');

    try {
      const response = await fetch('Natural Joy.kml?ts=' + Date.now(), { cache: 'no-store' });
      if (!response.ok) throw new Error('تعذر قراءة ملف KML');
      ...
      if (!polygons.length) throw new Error('لا توجد حدود مضلعة داخل ملف KML');

      const boundaryLayers = [];
      polygons.forEach(polygonNode => {
        const coordinatesNode = polygonNode.getElementsByTagNameNS('*', 'coordinates')[0];
        if (!coordinatesNode) return;
        const points = coordinatesNode.textContent.trim().split(/\s+/).map(entry => {
        ...
        polygon.bindPopup('<div class="land-popup-title">حدود أرض Natural Joy</div><div>اضغط على زر الاتجاهات للوصول إلى نقطة الدخول.</div>');
        boundaryLayers.push(polygon);
      });

      if (!boundaryLayers.length) throw new Error('تعذر استخراج إحداثيات حدود الأرض');
      const group = L.featureGroup([...boundaryLayers, entranceMarker]);
      map.fitBounds(group.getBounds(), { padding: [34, 34], maxZoom: 17 });
      status.hidden = true;
      setTimeout(() => map.invalidateSize(), 120);
    } catch (error) {
      status.textContent = 'تعذر عرض حدود الأرض: ' + error.message;
      status.classList.add('error');
      map.setView(entrance, 16);
    }
  }
