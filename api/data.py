...
def normalize_arabic(value: str) -> str:
    """Normalize Arabic text so categorization handles spelling variants reliably."""
...
def contains_any(text: str, keywords: list[str]) -> bool:
    return any(normalize_arabic(keyword) in text for keyword in keywords)


def category_for(name: str) -> str:
    """Assign the dashboard category using broad Arabic keyword coverage."""
    text = normalize_arabic(name)

    # Legal and purchase costs always belong to buying the land.
    if contains_any(text, [
        "ثمن الأرض", "ثمن الارض", "شراء الأرض", "شراء الارض", "عمولة", "غانم",
        "المحامي", "محامي", "مصاريف قانونية", "مصاريف قانونيه", "عقد", "تسجيل الأرض"
    ]):
        return "شراء الأرض"

    # Surveying, location preparation and civil site preparation.
    if contains_any(text, [
        "تحديد أماكن حفر البير", "تحديد اماكن حفر البير", "تحديد مكان حفر البئر",
        "تحديد أماكن حفر البئر", "أعمال المساحة", "اعمال المساحه", "اعمال مساحه",
        "تخطيط المزرعة", "تخطيط المزرعه", "تسوية", "تسويه", "طرق داخلية", "طرق داخليه",
        "أعمال مدنية", "اعمال مدنيه", "استراحة", "استراحه", "مخزن", "تجهيز الموقع"
    ]):
        return "تجهيز الموقع"

    # Advisory labour and recurring operations stay in services even when the name mentions agriculture.
    if contains_any(text, ["عمالة", "عماله", "عامل", "عمال", "استشارات", "استشاره", "إستشارات", "تشغيل", "خدمات"]):
        return "التشغيل والخدمات"

    # Planting, seedlings and field planting operations.
    if contains_any(text, [
        "حفر الجور", "الجور", "جور", "فسائل", "فسيلة", "فسيله", "نخل", "مجدول", "غنامي",
        "المشتل", "مشتل", "تحضين", "مصدات رياح", "مصد رياح", "كمبوست", "كومبوست",
        "سوبر فوسفات", "كبريت", "الزراعة", "الزراعه", "زراعة", "زراعه"
    ]):
        return "الزراعة والفسائل"

    # Pump/irrigation installation packages belong to water and irrigation, even
    # when their description also mentions a solar energy supply, motor or cables.
    # This captures workbook items such as:
    # "طلمبة وموتور + مواسير وكابلات الطلمبة".
    if contains_any(text, [
        "طلمبة وموتور", "طلمبه وموتور", "طلمبة و موتور", "طلمبه و موتور",
        "مواسير وكابلات الطلمبة", "مواسير وكابلات الطلمبه",
        "كابلات الطلمبة", "كابلات الطلمبه", "مواسير الطلمبة", "مواسير الطلمبه",
        "مضخة وموتور", "مضخه وموتور", "مواسير المضخة", "مواسير المضخه"
    ]):
        return "المياه والري"

    # Energy is deliberately separated from irrigation/water for standalone
    # energy infrastructure and generation items.
    if contains_any(text, [
        "طاقة", "طاقه", "الطاقة", "الطاقه", "طاقة شمسية", "طاقه شمسيه", "شمسي", "شمسية", "شمسيه",
        "كهرباء", "كهربائي", "كهربائية", "كهربائيه", "مولد", "مولدات", "انفرتر", "انفيرتر",
        "ألواح", "الواح", "لوح شمسي", "كابلات", "كابل", "موتور"
    ]):
        return "الطاقة"

    # Water infrastructure and irrigation systems.
    if contains_any(text, [
        "المياه", "مياه", "الماء", "ماء", "الري", "ري", "شبكة الري", "شبكه الري", "شبكات ري",
        "شبكات", "مواسير", "ماسورة", "ماسوره", "بئر", "بير", "حفر البئر", "حفر البير",
        "طلمبة", "طلمبه", "مضخة", "مضخه", "بركة", "بركه", "ترسيب", "خزان", "خزانات"
    ]):
        return "المياه والري"

    if contains_any(text, ["سيارة", "سياره", "نقل", "انتقالات", "مواصلات", "وقود"]):
        return "النقل"
    if contains_any(text, ["متفرقات", "ادارية", "إدارية", "مصروفات عامة", "مصروفات عامه"]):
        return "إدارية ومتفرقات"
    return "أخرى"


def numeric_cell(value):
    """Return a real numeric cell value (including zero), otherwise None."""
    if isinstance(value, (int, float)):
        return float(value)
    if value in (None, ""):
        return None
...
def find_labeled_number(matrix: list[list], aliases: list[str]) -> float:
    """Find a label anywhere in a sheet and return the nearest number on its row.

    The capital workbook uses an RTL layout and may place a unit cell between the
    label and the numeric value. Searching by distance makes the parser tolerant
    of future formatting changes without hard-coding a cell address.
    """
    normalized_aliases = [normalize_arabic(alias) for alias in aliases]
    for row_idx, row in enumerate(matrix):
        for col_idx, cell in enumerate(row):
            text = normalize_arabic(cell)
            if not text:
                continue
            if not any(text == alias or alias in text for alias in normalized_aliases):
                continue
            max_distance = max(5, len(row))
            for distance in range(1, max_distance + 1):
                # In the RTL sheet the value is usually visually to the left,
                # but check both directions to tolerate rearranged layouts.
                for candidate_col in (col_idx - distance, col_idx + distance):
                    if candidate_col < 0 or candidate_col >= len(row):
                        continue
                    number = numeric_cell(row[candidate_col])
...
def extract_dashboard_data(content: bytes) -> dict:
    ...
    for row_idx in range(1, min(len(capital), 9)):
        ...
        share = as_number(get(capital, row_idx, 2))
        paid = as_number(get(capital, row_idx, 3))
        remaining = as_number(get(capital, row_idx, 4))
        if partner and "الإجمالي" not in partner and (share != 0 or paid != 0 or remaining != 0):
            partners.append({"name": partner, "share": share, "paid": paid, "remaining": remaining})
    misc_items = []
    ...
    land_metrics = {
        "total_land_area": find_labeled_number(capital, ["إجمالي مساحة الأرض"]),
        "target_medjool_area": find_labeled_number(capital, ["المساحة المستهدفة لزراعة المجدول"]),
        "target_other_crops_area": find_labeled_number(capital, ["المساحة المستهدفة للزراعات الأخرى", "المساحة المتبقية للزراعات الأخرى"]),
        "remaining_other_crops_area": find_labeled_number(capital, ["المساحة المستهدفة للزراعات الأخرى", "المساحة المتبقية للزراعات الأخرى"]),
        "target_medjool_palms": find_labeled_number(capital, ["عدد نخيل المجدول المستهدف"]),
        "planted_medjool_palms": find_labeled_number(capital, ["عدد نخيل المجدول المزروع"]),
    }
    return {
