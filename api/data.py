...
def fetch_url(url: str) -> tuple[bytes, str]:
    request = Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 NaturalJoyDashboard/1.0",
...
def download_workbook() -> bytes:
    ...
    candidates = [append_download_parameter(ONEDRIVE_SHARE_URL)]
    try:
        _, resolved = fetch_url(ONEDRIVE_SHARE_URL)
        parsed = urlparse(resolved)
        if "onedrive.live.com" in parsed.netloc and parsed.query:
            candidates += [f"https://onedrive.live.com/download?{parsed.query}", append_download_parameter(resolved)]
    except Exception:
        pass
    errors = []
    for candidate in list(dict.fromkeys(candidates)):
        try:
            content, _ = fetch_url(candidate)
            if valid_xlsx_bytes(content):
                return content
            errors.append("OneDrive returned a page instead of a downloadable Excel file")
        except Exception as exc:
            errors.append(str(exc))
    raise RuntimeError("The OneDrive workbook could not be downloaded. Confirm that the shared link is viewable without sign-in. " + (errors[-1] if errors else ""))
